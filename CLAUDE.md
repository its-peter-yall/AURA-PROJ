...
Dual-application monorepo with two independent but related apps:

### AURA-CHAT (Student-Facing)
- **Stack:** React 19 + Vite + TypeScript (frontend), FastAPI + Neo4j + Vertex AI (backend)
...
- **Stack:** React 18 + Vite + TypeScript (frontend), FastAPI + Firestore + Deepgram (backend)
...
2. **Dual frontends in AURA-CHAT**: Use `client/` (React 19), NOT `frontend/` (Streamlit legacy)
3. **Split backend in AURA-CHAT**: Prefer `server/` (modern FastAPI) over `backend/` (legacy)
4. **4 CI/CD systems**: GitHub Actions + GitLab CI + Jenkins + Docker Compose — verify which system applies
...
### Frontend Styling
- Custom "Cyber Yellow" primary: `#FFD400`
...
- **5-minute timeout** for document processing endpoints
- **Read `.planning/BRIEF.md`** for architecture context
- **Parallelize** frontend work using the `frontend-ui-ux-engineer` agent for visual changes
